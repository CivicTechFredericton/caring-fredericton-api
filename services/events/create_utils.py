...
def create_event(**event_args):
    ...
    if event_args['is_recurring'] and not event_args['is_ending']:
        set_occurrences_recurring_not_ending(event_args)

    # Check if it is recurring and will go for ever [is_recurring=False and is_ending=False ]
    if not event_args['is_recurring'] and not event_args['is_ending']:
        message = 'An event cannot concurrently recur for ever and not be a recurring event'
        raise errors.ResourceValidationError(messages={'Recurring_Paradox': [message]})
...
def set_occurrences_recurring_ending(event_args):
    # For the case the event begins on the 28th of February, where peculiar bugs will emerge if not taken into account
    february_flag = False
    if event_args.get('start_date').month == 2 and event_args.get('start_date').day == 28:
        february_flag = True
...
def set_occurrences_recurring_not_ending(event_args):
    february_flag = False  # For the case the event begins on the 28th of February
    if event_args.get('start_date').month == 2 and event_args.get('start_date').day == 28:
        february_flag = True
...
def populate_occurrences(start_date, end_date, recurrence_details, february_flag):
...
def populate_occurrences_with_end_date(start_date, end_date, end_date_no_recur, recurrence_details, february_flag):
    day_separation, week_separation, month_separation = define_interval_increments(recurrence_details['recurrence'])
...
def populate_occurrences_for_ever(start_date, end_date, recurrence_details, february_flag):
    day_separation, week_separation, month_separation = define_interval_increments(recurrence_details['recurrence'])
...
def set_occurrence_date(start_date, end_date, day_separation, week_separation, month_separation, nday_separation,
                        nweek_separation, frequency, february_flag):
    ...
    if (nweek_separation != 0) and (nday_separation != 0) and (month_separation != 0):
        new_start_date = monthly_nday_nweek(start_date, month_separation, nday_separation, nweek_separation, frequency)
    ...
    # Here absolute intervals are supported = e.g day3 of next month if MONTHLY argument
        ...
        if day_separation != 0:
            new_start_date = start_date + relativedelta(days=+(day_separation*frequency))
        if week_separation != 0:
            new_start_date = start_date + relativedelta(weeks=+(week_separation*frequency))
        if month_separation != 0:
            if february_flag:
                new_start_date = start_date + relativedelta(day=31, months=+(month_separation * frequency))
            else:
                new_start_date = start_date + relativedelta(months=+(month_separation*frequency))
...
def monthly_nday_nweek(start_date, month_separation, nday_separation, nweek_separation, frequency):
    arg = MO(1)
    new_start_date = start_date

    if nday_separation == 1:
        arg = MO(nweek_separation)
    if nday_separation == 2:
        arg = TU(nweek_separation)
    if nday_separation == 3:
        arg = WE(nweek_separation)
    if nday_separation == 4:
        arg = TH(nweek_separation)
    if nday_separation == 5:
        arg = FR(nweek_separation)
    if nday_separation == 6:
        arg = SA(nweek_separation)
    if nday_separation == 7:
        arg = SU(nweek_separation)

    if nweek_separation != -1:
        new_start_date = start_date + relativedelta(day=1,
                                                    months=+(month_separation*frequency),
                                                    weekday=arg)
    if nweek_separation == -1:
