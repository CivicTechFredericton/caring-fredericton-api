...
def populate_occurrences(start_date, end_date, recurrence_details):
    ...
    if occurrence_type == constants.OccurrenceType.NEVER.value:
        ...
        print('Invalid Occurrence Type Value')

    # switcher = {
    #     constants.OccurrenceType.NEVER.value: generate_occurrence_type_never_events(start_date, end_date,
    #                                                                                 recurrence_details),
    #     constants.OccurrenceType.AFTER.value: generate_occurrence_type_after_events(start_date, end_date,
    #                                                                                 recurrence_details),
    #     constants.OccurrenceType.ON.value: generate_occurrence_type_on_events(start_date, end_date,
    #                                                                           recurrence_details)
    # }

    # Get the function from switcher dictionary
    # last_end_date, occurrences = switcher.get(occurrence_type, lambda: "Invalid Occurrence Type Value")
    for occurrence in occurrences:
        print(occurrence)

    return last_end_date, occurrences
    # return switcher.get(occurrence_type, lambda: "Invalid Occurrence Type Value")
...
def generate_occurrence_type_after_events(start_date, end_date, recurrence_details):
    ...
    day_of_week, week_of_month, frequency = get_relative_interval_details(recurrence_details)
...
def set_relative_occurrence_date(start_date, end_date, month_separation, day_of_week, week_of_month, frequency):
    """
    ...
    new_start_date = set_next_relative_date(start_date, day_of_week, week_of_month, month_separation, frequency)
    new_end_date = new_start_date + start_end_difference

    return new_start_date, new_end_date


def set_next_relative_date(start_date, day_separation, week_separation, month_separation, frequency):
    arg = MO(1)
    # new_start_date = start_date

    if day_separation == 1:
        arg = MO(week_separation)
    if day_separation == 2:
        arg = TU(week_separation)
    if day_separation == 3:
        arg = WE(week_separation)
    if day_separation == 4:
        arg = TH(week_separation)
    if day_separation == 5:
        arg = FR(week_separation)
    if day_separation == 6:
        arg = SA(week_separation)
    if day_separation == 7:
        arg = SU(week_separation)

    if week_separation == -1:
